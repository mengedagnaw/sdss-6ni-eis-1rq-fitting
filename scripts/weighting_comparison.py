...
# ------------------------------------------------------------
# Customized for current files:
#   AS, SR350, SR425, SR525, SA1150
#
# Physical model:
#   Rs - (Q || R)
#
...
# -----------------------------
INPUT_DIR = Path.cwd()
FALLBACK_INPUT_DIR = Path("/mnt/data")

FILE_CANDIDATES = {
    "AS": [
        "AS_replacement_EIS_ready.csv",
    ],
    "SR350": [
        "SR350_replacement_EIS_ready.csv",
    ],
    "SR425": [
        "SR425_replacement_EIS_ready.csv",
    ],
    "SR525": [
        "SR525_EIS_ready.csv",
    ],
...
PLOT_ORDER = ["AS", "SR350", "SR425", "SR525", "SA1150"]

FIT_OVERRIDES = {
    "AS":     {"trim_high": 3, "trim_low": 0, "use_robust_loss": False},
    "SR350":  {"trim_high": 0, "trim_low": 1, "use_robust_loss": True},
    "SR425":  {"trim_high": 2, "trim_low": 0, "use_robust_loss": True},
    "SR525":  {"trim_high": 0, "trim_low": 2, "use_robust_loss": True},
    "SA1150": {"trim_high": 0, "trim_low": 0, "use_robust_loss": False},
}

WEIGHTING_SCHEMES = ["modulus", "separate", "none"]

OUT_DIR = INPUT_DIR / "EIS_1RQ_weighting_comparison_final"
...
def plot_nyquist(out_dir, sample_name, weighting, z_exp, z_fit_dense):
    ...
    ax.xaxis.set_major_formatter(mticker.StrMethodFormatter("{x:.1f}" if major_step < 1 else "{x:.0f}"))
    ax.yaxis.set_major_formatter(mticker.StrMethodFormatter("{x:.1f}" if major_step < 1 else "{x:.0f}"))
